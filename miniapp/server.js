...
const configuredDataDir = String(process.env.DATA_DIR || ENV.DATA_DIR || '').trim();
const DB_PATH = path.resolve(configuredDataDir || BOT_DIR, 'database.json');
...
function readDb() {
  try {
    ...
    data.keys = data.keys && typeof data.keys === 'object' ? data.keys : {};
    data.applications = Array.isArray(data.applications) ? data.applications : [];
    data.mod_versions = Array.isArray(data.mod_versions) ? data.mod_versions : [];
    data.stats = data.stats && typeof data.stats === 'object' ? data.stats : {};
    return data;
  } catch (e) {
    if (e.code === 'ENOENT') return { users: {}, keys: {}, applications: [], mod_versions: [], stats: {} };
    throw new Error(`Не удалось прочитать базу данных ${DB_PATH}: ${e.message}`);
  }
}

function writeDb(data) {
  // Serialize writes and keep a .bak like the Python bot does.
  const write = dbWriteQueue.catch(() => {}).then(() => {
...
route('GET', '/api/moderator/(\\d+)', (ctx) => {
  ...
  const [, storedKey] = findKey(db, u.key_code);
  const key = storedKey ? { ...storedKey } : null;
  return ctx.json(200, {
...
route('POST', '/api/moderator/(\\d+)/(ban|unban|kick|delete)', async (ctx) => {
  ...
  if (action === 'delete') {
    delete db.users[uid];
    const [storedCode] = findKey(db, u.key_code);
    if (storedCode) delete db.keys[storedCode];
  }
  await writeDb(db);
...
function keyView(k) {
  return { key_code: k.key_code || k.key, target_nickname: k.target_nickname, role: k.role, mode: k.mode,
    days: k.days || 30, is_used: enabled(k.is_used), is_active: keyIsActive(k), created_at: k.created_at || '',
...
function keyIsActive(key) {
  ...
  const status = String(key.status || '').trim().toLowerCase();
  return enabled(key.is_active, true) && !enabled(key.revoked) && !enabled(key.disabled)
    && !['revoked', 'disabled', 'inactive', 'expired'].includes(status);
}
...
function findKey(db, code) {
  const wanted = canonicalKey(code);
  if (!wanted) return [null, null];
  for (const [storedCode, key] of Object.entries(db.keys || {})) {
    if (!key || typeof key !== 'object') continue;
    if ([storedCode, key.key, key.key_code].some(candidate => canonicalKey(candidate) === wanted)) return [storedCode, key];
  }
  return [null, null];
}
...
route('POST', '/api/keys/create', async (ctx) => {
  ...
  db.keys[code] = {
    ...
    is_used: 0, is_active: 1, created_by: ctx.userId, created_at: nowStr(),
  };
  await writeDb(db);
...
route('POST', '/api/keys/([A-Za-z0-9-]+)/(revoke|reset|delete|extend)', async (ctx) => {
  const db = requireAdmin(ctx);
  if (!db) return;
  const requestedCode = ctx.params[0], action = ctx.params[1];
  const [code, key] = findKey(db, requestedCode);
  if (!key) return ctx.json(404, { error: 'Ключ не найден' });
  if (action === 'revoke') {
    key.is_active = 0;
    key.revoked = 1;
    const u = Object.values(db.users).find(x => canonicalKey(x.key_code) === canonicalKey(code));
  ...
  if (action === 'reset') {
    ...
    delete key.revoked;
    delete key.disabled;
    if (['revoked', 'disabled', 'inactive', 'expired'].includes(String(key.status || '').trim().toLowerCase())) delete key.status;
    for (const u of Object.values(db.users)) {
      if (canonicalKey(u.key_code) === canonicalKey(code)) { u.is_approved = 0; delete u.key_code; }
    }
  }
...
route('GET', '/api/stats', (ctx) => {
  ...
  return ctx.json(200, {
    ...
    activeKeys: Object.values(db.keys || {}).filter(k => enabled(k.is_used) && keyIsActive(k)).length,
    totals, byRole, top,
  });
});
